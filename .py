...
class HandControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🤖 AI Hand Gesture Control")
        ...
        self.title_label = tk.Label(
            self.root, text="Computer Control with AI Hand Gestures",
            font=("Segoe UI", 18, "bold"), fg="#00ffcc", bg="#1a1a1a"
        )
        self.title_label.pack(pady=10)

        # Live feed frame with border
        self.video_frame = tk.Label(self.root, bg="#111111", bd=4, relief="ridge")
        ...
        # Custom button style
        btn_style = {"font": ("Segoe UI", 12, "bold"), "width": 15, "bd": 0, "bg": "#00ffcc", "fg": "#1a1a1a", "activebackground": "#00ffaa"}

        self.start_btn = tk.Button(self.btn_frame, text="▶️ Start", command=self.start, **btn_style)
        self.start_btn.grid(row=0, column=0, padx=15, pady=5)

        self.stop_btn = tk.Button(self.btn_frame, text="⏹️ Stop", state='disabled', command=self.stop, **btn_style)
        self.stop_btn.grid(row=0, column=1, padx=15, pady=5)

        # Mode indicator
        self.mode_label = tk.Label(self.root, text="", font=("Segoe UI", 14, "bold"), fg="#ffaa00", bg="#1a1a1a")
    ...
    def putText(self, img, mode, loc=None, color=(0, 255, 255)):
        if loc is None:
            text_size = cv2.getTextSize(str(mode), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, 2)[0]
            img_h, img_w = img.shape[:2]
            x = (img_w - text_size[0]) // 2
            y = img_h - 40
            loc = (x, y)
        cv2.putText(img, str(mode), loc, cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, color, 2)
        return img
